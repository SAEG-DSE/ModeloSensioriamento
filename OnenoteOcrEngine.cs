...
namespace journeyofcode.Images.OnenoteOCR
{
        ...
        : IOcrEngine, IDisposable
    {
        ...
        {
            var app = this.Try(() => new Application(), e => new OcrException("Erro de inicialização do OneNote", e));
            this._page = this.Try(() => new OnenotePage(app), e => new OcrException("Erro de initialização de pagina", e));
        }

        public string Recognize(Image image)
        {
            return this.Try(() => this.RecognizeIntern(image), e => new OcrException("Erro durante o reconhecimento", e));
        }
    ...
    internal sealed class OnenotePage
    {
        ...
        private void LoadOrCreatePage()
        {
            ...
            if (section == null)
                throw new OcrException("Seção não encontrada");
